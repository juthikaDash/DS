...
public:
    ...
    vector<vector<int> > pacificAtlantic(vector<vector<int> >& heights) {
        //Contrain is 1 <= m, n <= 200
        //So commenting the below boudry check
        ...
        //dfs call for left and right columns
        for(int i = 0; i<m; i++){
        ...
        //compaing both the ocean to find the cell that can go both ways
        vector<vector<int> > result;
        for(int i = 0; i<m; i++){
            for(int j = 0; j<n;j++){
                if(pacific[j][j] && atlantic[j][j] ){
